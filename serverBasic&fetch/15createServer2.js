...
const server = http.createServer((req, res) => {
    ...
    if(filePath.includes('/api/')){ //check whether this is a API call from myScript
        console.log('Gnerating api result.');
        if(filePath.includes('data')){
            res.writeHead(200, {'Content-Type': 'application/json'});
            return res.end(JSON.stringify({data: 'Hi'}));
        }
        if(filePath.includes('studentInfo')){
            res.writeHead(200, {'Content-Type': 'application/json'});
            return res.end(JSON.stringify({
                Fisrt_Name : 'Selwamuththu',
                Middle_Name : 'Kandasami',
                Address : 'Galle' 
            }));
        }
    }
